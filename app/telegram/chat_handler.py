...
class ChatHandler:
    """Handles channel discovery, sync pagination, ownership evaluation, and chat updates."""
    ...
    def __init__(
        ...
        self._search_handler = ChatSearchHandler(
            adapter=self._adapter,
            get_owned_chat=lambda cid: self._owned_chats.get(cid),
            get_supergroup=lambda sg_id: self._supergroups.get(sg_id),
            get_basic_group=lambda bg_id: self._basic_groups.get(bg_id),
            register_owned_chat=self._register_discovered_chat,
    ...
    def search_chats(self, query: str, limit: int = 100) -> None:
        self._search_handler.search_chats(query, limit)

    def process_search_results(self, chat_ids: list[int], search_id: str) -> None:
        self._search_handler.process_search_results(chat_ids, search_id)

    def process_chat_details_from_search(self, search_id: str, chat_id: int, chat: dict[str, Any]) -> None:
        self._raw_chats[chat_id] = chat
        self._search_handler.process_chat_details(search_id, chat_id, chat)

    def _register_discovered_chat(self, chat: OwnedChat) -> None:
        self._owned_chats[chat.id] = chat
        self._sync_and_emit()
